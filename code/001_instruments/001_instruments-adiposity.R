...
for (FILE in list_files){
  # make label
  label <- gsub("/data/GWAS_data/work/pulit_2018_PMID30239722//", "", FILE)
  label <- gsub(".giant-ukbb.meta-analysis.", "-", label)
  label <- gsub(".23May2018.txt.gz", "", label)
  # data
  ...
  data_clumped <- ld_clump(dat = data,
                           clump_kb = 10000, clump_r2 = 0.001, clump_p = 5e-9,
                           pop = "EUR",
                           access_token = NULL,
  ...
  nrow(data)
  nrow(data_clumped)
  write.table(data_clumped, paste0("analysis/001_instruments/adiposity/instruments_", label, ".txt"),
              row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
}

# combine ====
list_files <- list.files("analysis/001_instruments/adiposity/", pattern = "txt", full.names = T)
list_data <- lapply(list_files, fread)
data <- bind_rows(list_data, .id = "id")
write.table(data, "analysis/001_instruments/adiposity/instruments_adiposity.txt",
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
